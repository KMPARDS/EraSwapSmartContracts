...
contract NRTManager is Ownable, SignerRole{
    ...
    uint256 public releaseNrtTime; // variable to check release date
    uint256 public MonthlyReleaseNrt;
    uint256 public AnnualReleaseNrt;
  ...
  function sendTokens(string memory text,  address addr ,uint256 amount) internal returns (bool) {
  ...
  function receiveTokens(string memory text,  address fromAddr ,uint256 amount) internal returns (bool) {
   ...
   */
    function resetStaking() external returns(bool) {
        require(msg.sender == stakingContract , "should reset staking " );
        stakersBal = 0;
        return true;
    }
   ...
   */
   ...
   /**
   * @dev to reset buzzCafe Balance
   * @return true if success
   */
    function resetBuzzCafeBal() external returns(bool) {
        require(msg.sender == buzzCafe , "should reset buzzCafe " );
        buzzCafeBal = 0;
        return true;
    }
...
function burnTokens() internal returns (bool){
      ...
      if(temp >= burnTokenBal ){
          ...
          TotalCirculation.sub(burnTokenBal);
          burnTokenBal = 0;
      }
      else{
          burnTokenBal = burnTokenBal.sub(temp);
          TotalCirculation.sub(temp);
   ...
   */
    ...
    function receiveMonthlyNRT() external onlySigner() returns (bool) {
        ...
        TotalCirculation = TotalCirculation.add(MonthlyReleaseNrt);
        require(tokenContract.balanceOf(address(this))>=MonthlyReleaseNrt,"NRT_Manger should have token balance");
        require(distribute_NRT(MonthlyReleaseNrt));
        if(monthCount == 11){
            monthCount = 0;
   ...
   */
    function distribute_NRT(uint256 NRTBal) internal isNotZero(NRTBal) returns (bool){
        ...
        buzzCafeBal = buzzCafeBal.add(buzzCafeNRT);
        stakersNRT = (NRTBal.mul(15)).div(100);
        stakersBal = stakersBal.add(stakersNRT);
        timeSwappersNRT = (NRTBal.mul(325)).div(1000);
        timeSwappersBal = timeSwappersBal.add(timeSwappersNRT);
        ...
        require(sendTokens("BuzzCafe",buzzCafe,buzzCafeNRT),"Tokens should be succesfully send");
        require(sendTokens("staking contract",stakingContract,stakersNRT),"Tokens should be succesfully send");
        require(sendTokens("send timeSwappers",timeSwappers,timeSwappersNRT),"Tokens should be succesfully send");
    ...
    */
    ...
    constructor (address token, address[] memory pool) public{
        ...
        AnnualReleaseNrt = 819000000000000000000000000;
